...
class DataContentProvider(ContentProvider):
    ...
    def represent_as_structure(self) -> Optional[bool]:
        ...
        if '{' in self.text and '}' in self.text and '"' in self.text and ':' in self.text:
            try:
                ...
                # fallback
                logger.debug("Cannot parse as json %s:%s %s", type(exc), exc,
                             self.data[:32] if self.data else self.data)
            else:
            ...
            try:
                ...
                # fallback
                logger.debug("Cannot parse as ndjson %s:%s %s", type(exc), exc,
                             self.data[:32] if self.data else self.data)
                self.structure = None
            else:
                if self.__is_structure():
        ...
        try:
            ...
            # fallback
            logger.debug("Cannot parse as Python %s:%s %s", type(exc), exc, self.data[:32] if self.data else self.data)
        else:
            if self.__is_structure():
                return True
        ...
        try:
            ...
            # fallback
            logger.debug("Cannot parse as yaml %s:%s %s", type(exc), exc, self.data[:32] if self.data else self.data)
        else:
            if self.__is_structure():
                return True
        # # # None of above
    ...
    def represent_as_xml(self) -> Optional[bool]:
        ...
        try:
            ...
            # fallback
            logger.debug("Cannot parse as XML %s:%s %s", type(exc), exc, self.data[:32] if self.data else self.data)
        return None
    ...
    def represent_as_html(
        ...
        try:
            ...
            # fallback
            logger.debug("Cannot parse as HTML %s:%s %s", type(exc), exc, self.data[:32] if self.data else self.data)
        else:
            return bool(self.lines and self.line_numbers)
        return None
