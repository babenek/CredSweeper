...
def disagreement_check(df_all, x_full_features, y_full, jobs=1):
    ...
    tree_predictions = np.array([tree.predict(x_full_features)
                                 for tree in clf.estimators_])  # shape: (n_trees, n_samples)
    ...
    for n in most_controversial_idx:
        ...
        print(f"{str(n)} " + fore_style + f" {str(disagreement[n])} "
              f" {str(clf.predict(x_full_features[[n]])[0])} "
              f" {str(df_all.iloc[[n]].to_dict())} " + Style.RESET_ALL,
              flush=True)
